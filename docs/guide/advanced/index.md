...
![Ethernet II 格式封装的 LLDP 报文](/images/image_149.png)
...
![SNAP 格式封装的 LLDP 报文](/images/image_150.png)
...
![LLDPDU 的封装格式](/images/image_151.png)
...
## 组织定义 TLV、802.3 组织定义 TLV 和 LLDP-MED（Media Endpoint Discovery，媒体终端发现） TLV。
...
| 配置项 | 说明 |
| --- | --- |
| 状态 | Disabled:全局使能关闭 Enabled:全局使能 |
| 系统名 | 设备的名称，可以为空 |
| 系统描述 | 系统的描述，可以为空 |
...
| 配置项 | 说明 |
| --- | --- |
| 状态 | Disabled：既不发送也不接收 LLDPDU TxOnly: 只发送不接收 LLDPDU RxOnly: 只接收不发送 LLDPDU TxRx: 既发送也接收 LLDPDU |
| Chassis Subtype | Mac-address:表示MAC地址 If-alias:表示接口化名 If-name:表示接口名称 Ip-address:表示IP地址 Locally-assigned:表示本地配置 |
| 描述 | 显示当前配置的LLDP端口的名称 |
| Agent Circuit ID | 代理巡回标识 |
| Locally Assigned | 本地配置 |
| Port ID Subtype | Mac-address: 表示MAC地址 If-alias: 表示接口化名 If-name: 表示接口名称 Ip-address: 表示IP地址 Agt-circuit-id: 表示代理巡回标识 Locally-assigned: 表示本地配置 |
| Management Address Subtype | Mac-address:设备MAC地址 Ip-address:设备IP地址 |
| Basic Tlvs | port-description:端口描述符 system-description:系统描述符 management-address:管理地址 system-name:系统名 system-capabilities:系统能力 |
| 802.1 Tlvs | port-vlanid:端口vlanid ptcl-identity:协议id vid-digest:vid摘要 vlan-name:vlan名称 port-ptcl-vlanid:端口协议vlanid link-agg mgmt-vid:链路聚合管理vid |
...
![IGMP Snooping工作原理](/images/image_157.png)
...
![IGMP全局配置界面](/images/image_158.png)
...
| 配置项 | 配置项 | 说明 |
| --- | --- | --- |
| IGMP Snooping | 状态 | 开启/关闭IGMP Snooping功能，默认为关闭。 |
| IGMP Snooping | 丢弃未知名组播 | 开启/关闭丢弃未知名组播功能 未知组播数据报文是指在IGMP Snooping转发表中不存在对应转发表项的 那些组播数据报文： • 当使能丢弃未知组播数据报文功能时，交换机将丢弃所有收到的未知组 播数据报文 • 当禁止丢弃未知组播数据报文功能时，交换机将在未知组播数据报文所 属的VLAN内广播该报文 |
| IGMP Snooping | 拓扑变化抑制 | 开启/关闭拓扑变化抑制功能 |

4.2.2.2 IGMP 路由口配置说明

(1) 在导航栏中选择【高级】【二层】【IGMP Snooping配置】，进入IGMP路由口页面，如图4-11所示。

![IGMP路由口界面](/images/image_159.png)
...
| 配置项 | 配置项 | 说明 |
| --- | --- | --- |
| IGMP路由口 | VID | 组播表项所属VLAN的ID |
| IGMP路由口 | 接口 | 所有成员端口 |
| IGMP路由口 | 删除 | 删除该IGMP路由 |

(2) 单击“组播路由口”下方的【添加】按钮，进入IGMP路由口设置界面，如图4-12所示，配置VID，选择需要应用的端口。点击【确认】按钮完成配置。

![IGMP路由口配置界面](/images/image_160.png)
...
![IGMP静态组显示界面](/images/image_161.png)
...
| 配置项 | 配置项 | 说明 |
| --- | --- | --- |
| IGMP静态组 | VID | 组播表项所属VLAN的ID |
| IGMP静态组 | 组地址 | 组播组地址 |
| IGMP静态组 | 源地址 | 组播源地址 |
| IGMP静态组 | 接口 | 所有成员端口 |
| IGMP静态组 | 删除 | 删除该IGMP静态组 |

(2) 单击“静态组”下方的【添加】按钮，进入IGMP静态组配置界面，如图4-14所示。依次配置VID、组地址、源地址以及接口名称。点击【确认】按钮完成配置。

![IGMP静态组配置界面](/images/image_162.png)
...
知名单播：以太网交换机在MAC地址表中查到与报文的目的MAC地址和VLAN ID相对应的表项并且表项中的输出端口是唯一的，报文直接从表项对应的端口输出。

未知名广播：以太网交换机在地址表中没有找到目标MAC地址对应的表项，报文被送到所属的VLAN中除报文输入端口外的其他所有端口输出。
...
| 配置项 | 配置项 | 说明 |
| --- | --- | --- |
| 全局配置 | 老化时间 | <30,1000>，默认老化时间是300秒，MAC地址在最后一次更新300到600秒时间范围内被系统老化 |
| 全局配置 | 应用 | 点击配置生效 |
| 静态地址 | MAC地址 | 配置的静态MAC地址，格式如：00-00-00-00-00-01 |
| 静态地址 | VID | MAC地址的vlan属性 |
| 静态地址 | 接口 | MAC地址的端口属性 |
| 静态地址 | 操作 | 点击删除该静态MAC地址 |
| 过滤地址 | MAC地址 | 配置过滤MAC地址，格式如：00-00-00-00-00-01 |
| 过滤地址 | VID | MAC地址的vlan属性 |
| 过滤地址 | 操作 | 点击删除该过滤MAC地址 |
...
![DHCP Snooping全局配置](/images/image_170.png)

<p align="center"><em>图4-22 DHCP Snooping全局配置</em></p>

**表4-14 DHCP Snooping全局配置**
...
![DHCP Snooping信任口配置](/images/image_171.png)
...
![QinQ典型应用组网图](/images/image_172.png)
...
![QinQ的报文结构](/images/image_173.png)
...
基于IP的标准ACL：只根据数据包的源IP 地址制定规则。

基于IP的扩展ACL：根据数据包的源IP 地址、目的IP 地址、ETYPE、protocol制订规则。

基于MAC的 ACL：根据数据包的源MAC 地址、目的MAC 地址制订规则。

基于IPV6的ACL：根据数据包的源IPV6 地址、目的IPV6 地址、protocol等制订规则。
...
![ACL配置界面](/images/image_177.png)

<p align="center"><em>图4-29 ACL配置界面</em></p>
...
| 配置项 | 配置项 | 说明 |
| --- | --- | --- |
| ACL类型 | IP | 标准IP的ACL，可匹配IPv4报文中的源IP字段 |
| ACL类型 | IP-Extend | 扩展ACL，可匹配IPv4报文的协议号、源IP地址、目的IP地址、4层端口号等 |
| ACL类型 | IPV6 | IPv6 ACL，可匹配IPv6报文源IP地址、目的IP地址、协议号等 |
| ACL类型 | MAC | MAC ACL，可匹配目的MAC地址、源MAC地址、Etype等字段 |
| 名称 | 名称 | 标准IP有效数字范围：<1-99> \| <1300-1999> 扩展IP有效数字范围：<100-199> \| <2000-2699> MAC ACL有效数字范围：<200-699> IPv6 ACL仅支持字符串命名，所有ACL均支持字符串命名，字符串长度范围<1-64> |
| 计数使能 | 计数使能 | 使能计数功能，当报文命中ACL时，计数值加1 |
| 起始序号 | 起始序号 | 规则表项序号起始值，缺省值:10, 范围<1-2147483647> |
| 步长 | 步长 | 序号地增值，缺省值:10, 范围<1-2147483647> |
| 描述符 | 描述符 | 定义该ACL描述信息 |

![ACL类型配置界面](/images/image_178.png)
...
| 访问控制 | Deny | 丢弃命中该规则的报文 |
| 序列号 | 序列号 | 规则表项序号 |
| 源地址 | 源地址 | 源IP地址，如192.168.64.1 |
| 源掩码 | 源掩码 | IP的掩码取反，如匹配IP地址前24位，掩码为255.255.255.0，这里需配置为00.00.00.255 |

![IP类型ACE配置界面](/images/image_179.png)

<p align="center"><em>图4-31 IP类型ACE配置界面</em></p>

**表4-20 IP-Extend类型ACE参数说明**
...
| 访问控制 | Deny | 丢弃命中该规则的报文 |
| 序列号 | 序列号 | 规则表项序号 |
| 协议 | 协议 | 支持常用协议报文选项，包含tcp、udp、vrrp、igmp、gre、ipcomp、ospf、pim、rsvp等 支持所有IPv4报文 支持自定义protocol的IPv4报文 |
| 源地址 | 源地址 | 源IP地址，如192.168.64.1 |
| 源掩码 | 源掩码 | IP的掩码取反，如匹配IP地址前24位，掩码为255.255.255.0，这里需配置为00.00.00.255 |
| 目标地址 | 目标地址 | 目的IP地址，如192.168.64.100 |
| 目标掩码 | 目标掩码 | 同源掩码 |

![IP-Extend类型ACE配置界面](/images/image_180.png)
...
| 访问控制 | Deny | 丢弃命中该规则的报文 |
| 序列号 | 序列号 | 规则表项序号 |
| 协议 | 协议 | 支持常用协议报文选项，包含tcp、udp、icmp等 支持所有IPv6报文 支持自定义protocol的IPv6报文 |
| 源地址 | 源地址 | 源MAC地址，如00.d0.f8.22.33.40 |
| 源掩码 | 源掩码 | MAC地址掩码取反，如匹配MAC地址前24位，掩码为ffff.ff00.0000，这里需配置为0000.00ff.ffff |
| 目标地址 | 目标地址 | 目的MAC地址，如00.d0.f8.22.33.41 |
| 目标掩码 | 目标掩码 | 同源掩码 |

![IPV6类型ACE配置界面](/images/image_181.png)
...
| --- | --- | --- |
| 访问 | Deny | 放行命中该规则的报文 |
| 访问 | Permit | 丢弃命中该规则的报文 |
| 序列号 | 序列号 | 规则表项序号 |
| 以太网协议 | 以太网协议 | 以太网协议类型 ，范围(0x05DD-0xFFFF) |
| 优先级 | 优先级 | 报文的cos值，范围（0-7） |
| 源地址 | 源地址 | 报文源MAC地址 |
| 源掩码 | 源掩码 | 源MAC地址掩码取反，比如选择MAC地址的前32位，掩码为0000.0000.ffff |
| 目的地址 | 目的地址 | 报文目的MAC地址 |  |
| 目的掩码 | 目的掩码 | 目的MAC地址掩码取反，比如选择MAC地址的前32位，掩码为0000.0000.ffff |  |

![MAC类型ACE配置界面](/images/image_182.png)
...
![QoS全局配置界面](/images/image_187.png)
...
| 配置项 | 说明 | 说明 | 说明 |
| --- | --- | --- | --- |
| 全局配置 | 状态 | 状态 | 启用QOS，在启用前所有QOS功能不支持配置 |
| 全局配置 | 算法 | Sp | 绝对优先级调度，队列ID大优先级高，高优先级队列处理完成后处理低优先级队列 |
| 全局配置 | 算法 | Wrr | 轮转调度算法，根据队列权重，从队列ID最大到最小，依次调度各个队列。 |

配置Qos映射

（1）在当前界面，点击“QoS映射”下方的【队列】按钮，进入队列配置界面，如图4-40所示，参数说明如表格4-24所示。

![队列配置界面](/images/image_188.png)
...
| --- | --- | --- |
| 队列权重 | 队列 | <0， 7> |
| 队列权重 | 权重 | <0， 32>，数值越大，权重越高，在通道拥堵情况下该队列报文优先处理概率越大，0表示无穷大。 |

（2）点击“QoS映射”下方的【CoS】按钮，进入CoS配置界面，如图4-41所示，参数说明如表格4-25所示。

![CoS配置界面](/images/image_189.png)
...
| --- | --- | --- |
| CoS配置 | CoS | <0， 7> |
| CoS配置 | 队列 | <0， 7>， Cos-queue映射关系，在端口标记的cos基础上，修改报文出口队列，在配置端口为no trust、trust cos或trust dscp且非ip报文时生效。 |
| CoS配置 | DSCP | cos-dscp映射关系，在配置端口为no trust、trust cos或trust dscp且非ip报文时生效，修改报文dscp数值 |

（3）点击“QoS映射”下方的【DSCP】按钮，进入DSCP配置界面，如图4-42所示，参数说明如表格4-26所示。

![DSCP配置界面](/images/image_190.png)
...
| --- | --- | --- |
| DSCP映射 | DSCP | <0， 63> |
| DSCP映射 | 队列 | <0， 7>， dsp-queue映射关系，在配置端口为trust dscp且ip报文时生效，修改报文出口队列 |
| DSCP映射 | CoS | <0， 7>， dscp-cos映射关系，在配置端口为trust dscp且ip报文时生效，修改报文cos字段 |
| DSCP映射 | 新DSCP值 | <0， 63>， dscp-dscp映射关系，在配置端口为trust dscp且ip报文时生效，先进行dscp-dscp映射，后进行dscp-cos映射 |

Qos分类配置

在当前QoS界面，点击“分类配置”下方的【添加】按钮，进入分类配置界面，如图4-43所示，参数说明如表格4-27所示。

![图4-43分类配置界面](/images/image_191.png)
...
| --- | --- | --- |
| 分类配置 | 名称 | 创建分类，定义分类名称 |
| 分类配置 | 匹配类型 | 定义匹配类型，支持关联acl，etype，dscp，cos，l4，vlan-rangge，vlan |

Qos策略配置

在当前QoS界面，点击“策略配置”下方的【添加策略规则】按钮，进入规则配置界面，如图4-44所示，参数说明如表格4-28所示。

![规则配置界面](/images/image_192.png)
...
| 规则配置 | 名称 | 规则名称 |
| 规则配置 | 分类名 | 选择分类名称 |
| 规则配置 | 修改 | 策略对应的动作一，支持修改cos、dscp、vlan等动作 |
| 规则配置 | 限速 | 策略对应的动作二，限速 |
| 规则配置 | CIR | 限速水线，单位kbps |
| 规则配置 | CBS | burst能力，单位Kbyte |
...
| 配置项 | 配置项 | 说明 |
| --- | --- | --- |
| 端口配置 | 默认CoS | <0， 7>，当配置端口不信任，或配置信任但报文不满足信任条件时，采用端口默认cos标记入口报文 |
| 端口配置 | 信任 | 支持不信任、信任cos、信任dscp配置。当处于no trust模式时，入口阶段根据端口默认cos修改报文cos字段以及dscp字段；当配置trust cos时，对于untagged报文同no trust模式，对于带tag报文，选择报文自带cos；当配置trust dscp时，对于ip报文，选择报文自带dscp，对于非ip报文，同trust cos模式。 |
| 端口配置 | 入口策略 | 选择创建的策略 |

![QoS端口配置界面](/images/image_193.png)
...
![DoS全局配置界面](/images/image_194.png)
...
![创建全局防ARP Flood攻击类型](/images/image_196.png)

<p align="center"><em>图4-48 创建全局防ARP Flood攻击类型</em></p>

创建成功后的DoS全局配置界面如图4-49所示。

![创建全局防ARP Flood攻击类型](/images/image_197.png)
...
**表4-31 防SYN/ICMP/ARP Flood攻击类型参数说明**
...
![SYN配置界面](/images/image_198.png)

<p align="center"><em>图4-50 SYN配置界面</em></p>

![ICMP配置界面](/images/image_199.png)

<p align="center"><em>图4-51 ICMP配置界面</em></p>

![ARP配置界面](/images/image_200.png)
...
![防ARP Flood配置界面](/images/image_204.png)
...
![ARP/邻居表项信息](/images/image_205.png)
...
![ARP/邻居配置界面](/images/image_206.png)

<p align="center"><em>图4-58 ARP/邻居配置界面</em></p>

![图4-59新建ARP/邻居界面](/images/image_207.png)
...
![ARP/邻居配置页面](/images/image_211.png)
